...
def build_state_with_status_debug(machine, rule, old=None):
    state = _original_build_state(machine, rule, old)
    name = str(state.get("name", ""))
    if "CARREFOURSA" in name.upper() and "BODRUM" in name.upper():
        print(
            "[DEBUG-KULLANIM-DURUMU]",
            {
                "machineId": machine.get("id"),
                "name": name,
                "active": state.get("active"),
                "machineStatus": state.get("machineStatus"),
...
scraper.build_state = build_state_with_status_debug
